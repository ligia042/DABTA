		...
		try {
			...
			if (simulator == "sumo") {
				...
				if(OS.indexOf("win") >= 0){
					...
					command.add("/Users/ligiacp/Downloads/sumo-0.18.0/bin/sumo-gui");
				}
				command.addAll(params);
			}
...
//				System.out.println("ADDED VEHICLE!");
	...
	public static void createAllRoutesLigiaSimple() {
	...
	public void subscribeEdgesVehicleIDs() {
		...
		for(int i = 1; i <= 7; i++){
			...
			Content cnt = new Content(simStartStep,simEndStep,"e"+i,variableList);
